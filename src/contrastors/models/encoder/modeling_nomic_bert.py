...
class NomicBertEncoder(NomicBertPreTrainedModel):
    ...
    def forward(
        ...
        do_unpad = not self.gradient_checkpointing and getattr(self.config, "moe_every_n_layers", 0) <= 0

        if do_unpad:
            hidden_states, indices, cu_seqlens, max_seqlen_in_batch, *rest = unpad_input(
        ...
        all_router_logits = []
        all_layer_hidden_states = [] if output_hidden_states else None
        ...
        # ✅ Restore padded shape after MoE unpadding
        if indices is not None:
            hidden_states = pad_input(hidden_states, indices, batch, seqlen)

        # At this point hidden_states is [batch, seqlen, dim] again, matching original attention_mask

        if output_hidden_states:
            return hidden_states, all_router_logits, tuple(all_layer_hidden_states)
        else:
            return hidden_states, all_router_logits, None
...
# Patch: Make NomicBertModel initialize the MLM head and tie weights like NomicBertForPreTraining
...
class NomicBertModel(NomicBertPreTrainedModel):
    ...
    def __init__(self, config: GPT2Config, add_pooling_layer=True):
        ...
        # === MLM head and weight tying as in NomicBertForPreTraining ===
        self.cls = NomicBertPreTrainingHeads(config)

        # MLM loss: prefer flash_attn's fused CrossEntropyLoss if available
        ...
        # === Domain-restricted predictions ===
        # If domain_token_ids_path is set, load domain token IDs and build a mask
        # so that MLM predictions are restricted to domain tokens only.
        # Non-domain logits are set to -inf before loss computation.
        domain_token_ids_path = getattr(config, "domain_token_ids_path", None)
        if domain_token_ids_path is not None:
            with open(domain_token_ids_path) as f:
                domain_token_ids = json.load(f)
            # Boolean mask: True = domain token (keep), False = non-domain (mask to -inf)
            domain_vocab_mask = torch.zeros(config.vocab_size, dtype=torch.bool)
            for tid in domain_token_ids:
                if tid < config.vocab_size:
                    domain_vocab_mask[tid] = True
            # Register as buffer so it moves with .to(device) and isn't a parameter
            self.register_buffer("domain_vocab_mask", domain_vocab_mask)
            logger.info(
                f"Domain-restricted MLM: {domain_vocab_mask.sum().item()} / {config.vocab_size} "
                f"tokens active for prediction"
            )
        else:
            self.domain_vocab_mask = None

        self.apply(partial(_init_weights, initializer_range=config.initializer_range))
        self.tie_weights()  # <-- IMPORTANT: tie the MLM head weights
    ...
    def forward(
        ...
        sequence_output, router_outputs, all_hidden_states = self.encoder(
            hidden_states,
            attention_mask=attention_mask,
            output_hidden_states=output_hidden_states,
        )

        # === MLM: compute loss on masked tokens only (dense_seq_output) ===
        mlm_loss = None
        if labels is not None:
            masked_token_idx = torch.nonzero(labels.flatten() >= 0, as_tuple=False).flatten()
            if masked_token_idx.numel() > 0:
                masked_hidden = index_first_axis(
                    rearrange(sequence_output, "b s d -> (b s) d"),
                    masked_token_idx,
                )
                prediction_scores = self.cls(masked_hidden)

                # Domain-restricted predictions: mask non-domain logits to -inf
                if self.domain_vocab_mask is not None:
                    prediction_scores = prediction_scores.masked_fill(
                        ~self.domain_vocab_mask.unsqueeze(0), float("-inf")
                    )

                masked_labels = labels.flatten()[masked_token_idx]
                mlm_loss = self.mlm_loss(prediction_scores, masked_labels).float()
            else:
