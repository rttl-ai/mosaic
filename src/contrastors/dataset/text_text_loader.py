...
class DomainTokenOnlyMaskingCollator:
    def __init__(self, tokenizer, domain_token_ids: Set[int], mlm_probability=0.15):
        self.tokenizer = tokenizer
        self.domain_token_ids = domain_token_ids
        self.mlm_probability = mlm_probability
        # Pre-build a boolean lookup tensor for vectorized domain membership check
        special_ids = {tokenizer.cls_token_id, tokenizer.sep_token_id, tokenizer.pad_token_id}
        special_ids.discard(None)
        vocab_size = len(tokenizer)
        self._is_domain = torch.zeros(vocab_size, dtype=torch.bool)
        for tid in domain_token_ids - special_ids:
            self._is_domain[tid] = True

    def __call__(self, examples: List[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
        input_ids = torch.stack([ex['input_ids'] for ex in examples])
        labels = input_ids.clone().fill_(-100)

        # Vectorized domain membership: (batch, seq_len) bool
        domain_mask = self._is_domain[input_ids]

        # Random masking probability
        rand_mask = torch.rand_like(input_ids, dtype=torch.float) < self.mlm_probability
        mask = domain_mask & rand_mask

        # Set labels at masked positions to original token ids
        labels[mask] = input_ids[mask]

        # 80% → [MASK], 10% → random, 10% → unchanged
        probs = torch.rand_like(input_ids, dtype=torch.float)
        mask_token = probs < 0.8
        random_token = (probs >= 0.8) & (probs < 0.9)

        input_ids[mask & mask_token] = self.tokenizer.mask_token_id
        input_ids[mask & random_token] = torch.randint(len(self.tokenizer), (int((mask & random_token).sum()),))
...
class StreamingShardDataset(IterableDataset):
    def __init__(
        ...
        #self.mlm_collator = DataCollatorForLanguageModeling(tokenizer, mlm_probability=0.15)

        if domain_token_ids_path is not None:
            with open(domain_token_ids_path) as f:
                domain_token_ids = set(json.load(f))
            self.mlm_collator = DomainTokenOnlyMaskingCollator(tokenizer, domain_token_ids, mlm_probability=0.15)
        else:
    ...
    def normalize_url(self, urls):
        ...
        for url in urls:
            ...
            split = url.split("/")
            if len(split) >= 6:
                normed = "/".join(split[-10:]) # support long path by mm@rttl.ai
            else:
                normed = "/".join(split[-3:])
            norm_urls.append(normed)
    ...
    def tokenize_pairs(self, samples, objective):
        ...
        for col in MAPPED_NAMES[contrastive_type]:
            ...
            tokenized_inputs[f"{col}_input_ids"] = tokenized["input_ids"]
            tokenized_inputs[f"{col}_attention_mask"] = tokenized["attention_mask"]
            
            # === PATCH: Apply MLM to ONLY documents, NOT queries ===
            if col == "document" and self.mlm_collator is not None:
                # Only input_ids is required by the custom collator
                mlm_batch = self.mlm_collator([
                    {"input_ids": ids} for ids in tokenized["input_ids"]
                ])
                tokenized_inputs[f"{col}_input_ids"] = mlm_batch["input_ids"]  # Overwrite with masked inputs
                tokenized_inputs[f"{col}_labels"] = mlm_batch["labels"]
                # Attention mask is left as original (from tokenizer)
            ...
            #     ])
            #     tokenized_inputs[f"{col}_input_ids"] = mlm_batch["input_ids"]  # Overwrite with masked inputs
            #     tokenized_inputs[f"{col}_attention_mask"] = mlm_batch["attention_mask"]
            #     tokenized_inputs[f"{col}_labels"] = mlm_batch["labels"]
            #     # Highlighted for review: MLM is NOT applied to queries anymore
